...
public class BlenderCraftingManager : MonoBehaviour
{
    ...
    public ItemScriptableObject emptyBottleData; 
    public ItemScriptableObject orangeJuiceData;  
    ...
    public void Blend()
    {
        ...
        for (int i = 1; i <= 2; i++) // Проходим по всем слотам
        {
            GameObject slot = GameObject.Find("BlenderSlot" + i);
    ...
    {
        blendButton.interactable = false; // Отключаем кнопку во время крафта
        ClearIngredients(); // Очищаем ингредиенты
        arrowAnimator.SetTrigger("StartCrafting");
        yield return new WaitForSeconds(3); // Имитация процесса крафта
        ...
        blendButton.interactable = true; // Включаем кнопку после крафта
    }
    ...
    private void ClearIngredients()
    {
        for (int i = 1; i <= 2; i++)
        {
            GameObject slotGameObject = GameObject.Find("BlenderSlot" + i);
            ...
            {
                Debug.LogError("Slot GameObject not found: BlenderSlot" + i);
            }
        }
    }
    ...
    private void Start()
    {
        ...
        itemsDictionary.Add("EmptyBottle", emptyBottleData);
        itemsDictionary.Add("OrangeJuice", orangeJuiceData);
        
        // Рецепт "Апельсиновый сок"
        recipes.Add(new Recipe() { ingredients = new List<string> { "Empty_Bottle", "Sliced_Orange" }, result = "OrangeJuice" });
    }
}